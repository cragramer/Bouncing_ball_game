...
u8 eat = 0;
u8 press = 0;
u16 stick_x = 240, stick_y = 400;
u8 stick_wide = 100, stick_high = 10;
int stick_speed = 5;
u8 speedup = 0;
...
void direction_lookup(u8 DIRECTION) {
	...
	if (ball_y_speed > 0) { ball_y_speed += 1; }
	else { ball_y_speed -= 1; }
}
...
void move_the_ball(int speedx, int speedy) {
	...
	ball_x_old = ball_x;
	ball_y_old = ball_y;
	if (key_up_on() == 1 || JOYPAD_Read() == 1 << 4) { speedup = 1; }
	else { speedup = 0; }
	if (game_end == 1) { return; }
	eat = 0;
	//judge if the ball hit the stick
	if (hit_the_stick()) {
		ball_y_speed = -ball_y_speed;
		eat = 1;
	}
	...
	//move the ball
	if (speedup == 1) {
		if (ball_x_speed > 0) { ball_x_speed = ball_x_speed + 2; }
		else { ball_x_speed = ball_x_speed - 2; }
		if (ball_y_speed > 0) { ball_y_speed = ball_y_speed + 2; }
		else { ball_y_speed = ball_y_speed - 2; }
	}
	ball_x = ball_x + ball_x_speed;
	ball_y = ball_y + ball_y_speed;
	//draw the ball
	if (speedup == 1) {
		if (ball_x_speed > 1) { ball_x_speed = ball_x_speed - 2; }
		else { ball_x_speed = ball_x_speed + 2; }
		if (ball_y_speed > 1) { ball_y_speed = ball_y_speed - 2; }
		else { ball_y_speed = ball_y_speed + 2; }
	}
	draw_the_ball(BLACK);
	if (number_of_bounce % 5 == 4) { draw_the_ball(WHITE); }
	//if (eat==1){
...
/* bounus part*/
/*.....................................................*/
// draw a stick in the middle of the screen
void draw_the_stick(u16 x, u16 y, u16 color) {
	EIE3810_TFTLCD_FillRectangle(x - stick_wide / 2, stick_wide, y - stick_high / 2, stick_high, color);
}

//hit the stick
int hit_the_stick() {
	if (ball_x + ball_x_speed >= stick_x - stick_wide / 2 &&
		ball_x + ball_x_speed <= stick_x + stick_wide / 2 &&
		ball_y + ball_y_speed >= stick_y - stick_high / 2 - ball_radius &&
		ball_y + ball_y_speed <= stick_y + stick_high / 2 + ball_radius
		)
	{
...
void move_stick() {
	// draw the stick
	draw_the_stick(stick_x, stick_y, WHITE);
	if (stick_x <= stick_wide / 2 + 20) {
		stick_speed = -stick_speed;
	}
	if (stick_x >= 480 - stick_wide / 2 - 20) {
		draw_the_stick(stick_x, stick_y, WHITE);
		stick_speed = -stick_speed;
	}
	stick_x += stick_speed;
	draw_the_stick(stick_x, stick_y, BLACK);
}
